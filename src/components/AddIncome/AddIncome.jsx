...
import { useAddIncomeMutation, useRepeatTransaction } from "@/reactQuery/queryMutatuion";
...
let {mutateAsync : addIncome,isPending : isAddingINcome,isError : isAddIncomeErr} = useAddIncomeMutation()
let {mutateAsync : repeatTransaction,isPending:isRepeating} = useRepeatTransaction()
  ...
  let submitForm = async (data) => {
   ...
   if(repeat == false){
    ...
    console.log(data);
    addIncomeToWallet = await addIncome({
    ...
    if(addIncomeToWallet){
        console.log(addIncomeToWallet);
  ...
  return (
    ...
    transition={pageTransition} onSubmit={handleSubmit(submitForm)}>
      <div className="min-h-full w-full bg-[#00A86B]">
        <BackTexttemplate
      ...
      <div className="relative w-full  bottom-36 bg-white min-h-36 pt-11 rounded-tl-[2rem] rounded-tr-[2rem]">
        <div className=" m-auto w-[90%] flex flex-col gap-y-10">
          ...
          <ThemeButton isPending={isAddingINcome} type="submit">Continue</ThemeButton>
        </div>
      </div>
    </motion.form>
  );
